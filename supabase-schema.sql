...
insert into public.menu_items (
  id, name, description, detail, price, category, image_url, tags, recommended, sold_out, option_groups, sort_order, active
) values
  (1, '十全大补汤', '温润药材慢炖汤底，适合夜宵后补气暖胃。', '以党参、当归、黄芪等温补药材慢火熬煮，汤底清润不腻，适合想要暖胃补气的夜宵时段。可按口味加饭或加肉。', 28, '炖汤', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/1-dish.jpg', array['招牌', '慢炖'], true, false, '[{"id":"portion","name":"加量选择","type":"multiple","options":[{"id":"rice","name":"加白饭","priceDelta":3},{"id":"pork","name":"加肉片","priceDelta":6}]}]'::jsonb, 10, true),
  (3, '冬阴功海鲜汤', '酸辣泰式汤底配海鲜，入口鲜明开胃。', '泰式香料、香茅、南姜与青柠熬出鲜明酸辣汤底，搭配虾、鱿鱼与贝类。可调整辣度，也可额外加海鲜。', 38, '泰式菜', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/3-dish.jpg', array['酸辣', '海鲜'], true, false, '[{"id":"spice","name":"辣度","type":"single","options":[{"id":"mild","name":"微辣","priceDelta":0},{"id":"regular","name":"正常辣","priceDelta":0},{"id":"extra","name":"加辣","priceDelta":1}]},{"id":"add-on","name":"加料","type":"multiple","options":[{"id":"shrimp","name":"加虾","priceDelta":8},{"id":"mushroom","name":"加蘑菇","priceDelta":4}]}]'::jsonb, 20, true),
  (4, '泰式罗勒叶炒肉', '罗勒香气浓郁，适合配饭的热炒主菜。', '大火快炒猪肉碎、罗勒叶、蒜末与泰式酱汁，香气浓郁，默认微辣，适合作为配饭主菜。', 22, '泰式菜', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/4-dish.jpg', array['热炒', '微辣'], false, false, '[{"id":"add-on","name":"加料","type":"multiple","options":[{"id":"egg","name":"加煎蛋","priceDelta":3},{"id":"rice","name":"加白饭","priceDelta":3}]}]'::jsonb, 30, true),
  (5, '越南牛肉粉', '清香牛骨汤底，搭配河粉和牛肉片。', '牛骨汤底长时间熬煮，配河粉、牛肉片、香草与青柠。汤感清爽，适合想吃主食但不想太重口味的顾客。', 26, '越南菜', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/5-dish.jpg', array['汤粉', '清爽'], false, false, '[{"id":"add-on","name":"加料","type":"multiple","options":[{"id":"beef","name":"加牛肉片","priceDelta":7},{"id":"noodle","name":"加河粉","priceDelta":4}]}]'::jsonb, 40, true),
  (6, '越南春卷', '清爽蔬菜与米纸卷，适合分享的小食。', '米纸包裹新鲜蔬菜、香草与爽口配料，搭配蘸酱食用。适合多人分享或作为清爽前菜。', 18, '越南菜', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/6-dish.jpg', array['小食', '清爽'], false, false, '[{"id":"sauce","name":"蘸酱","type":"single","options":[{"id":"peanut","name":"花生酱","priceDelta":0},{"id":"fish","name":"鱼露酸甜酱","priceDelta":0}]}]'::jsonb, 50, true),
  (7, '红毛榴莲水', '红毛榴莲果香清甜，调成清爽果饮，入口顺滑不腻。', '以红毛榴莲果肉风味调制，带自然热带果香与柔和甜感，冰饮更清爽，适合搭配热菜或饭后解腻。', 12, '饮料', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/hong-mao-liu-lian-shui.jpg', array['果香', '清爽'], true, false, '[]'::jsonb, 60, true),
  (8, '咸柠七', '咸柠檬配汽水，解腻醒胃。', '咸柠檬与七喜调制，咸香带酸甜，适合搭配重口味热炒或酸辣汤。', 9, '饮料', 'https://dknmznkimydnkcmbejqi.supabase.co/storage/v1/object/public/menu-items/8-dish.jpg', array['冰饮', '清爽'], false, false, '[{"id":"ice","name":"冰量","type":"single","options":[{"id":"less","name":"少冰","priceDelta":0},{"id":"regular","name":"正常冰","priceDelta":0}]}]'::jsonb, 70, true)
...
insert into public.menu_categories (label, sort_order, active)
select category, 1000 + row_number() over (order by category) * 10, true
from (
...
where not exists (
  select 1 from public.menu_categories c where c.label = existing.category
);
...
where item.category = category.label
  and item.category_id is distinct from category.id;
...
from (
  values
    ...
    (8, $${
      ...
      "vi": {"name":"7-Up chanh muối","description":"Chanh muối pha soda, thanh mát và đỡ ngấy.","detail":"Chanh muối pha cùng 7-Up, vị mặn thơm chua ngọt. Hợp dùng với món xào đậm vị hoặc súp chua cay.","category":"Đồ uống","tags":["Đồ uống đá","Thanh mát"],"optionGroups":[{"id":"ice","name":"Lượng đá","options":[{"id":"less","name":"Ít đá"},{"id":"regular","name":"Đá thường"}]}]}
    }$$::jsonb)
...
set name = '海底椰龙眼水',
    ...
    tags = array['清润', '龙眼'],
    translations = $${
      "en": {"name":"Sea Coconut Longan Drink","description":"Sea coconut and longan brewed into a gentle, throat-soothing sweet drink.","detail":"Slow-brewed with sea coconut and longan for a clear, soothing drink with light fruit sweetness and herbal aroma. Smooth and refreshing after meals or on hot days.","category":"Herbal Drinks","tags":["Soothing","Longan"]},
      "th": {"name":"น้ำมะพร้าวทะเลลำไย","description":"มะพร้าวทะเลต้มกับลำไย หวานใส ชุ่มคอ รสนุ่ม","detail":"ต้มมะพร้าวทะเลกับลำไยเป็นเครื่องดื่มชุ่มคอ มีกลิ่นสมุนไพรอ่อนๆ และความหวานจากผลไม้ ดื่มง่าย เหมาะหลังอาหารหรือวันที่อากาศร้อน","category":"เครื่องดื่มสมุนไพร","tags":["ชุ่มคอ","ลำไย"]},
      "vi": {"name":"Nước dừa biển nhãn nhục","description":"Dừa biển nấu cùng long nhãn, ngọt thanh và dịu cổ.","detail":"Dừa biển và long nhãn được nấu chậm thành thức uống thanh mát, có vị ngọt trái cây nhẹ và hương thảo mộc dịu. Phù hợp sau bữa ăn hoặc trong ngày nóng.","category":"Đồ uống thảo mộc","tags":["Thanh nhuận","Long nhãn"]}
    }$$::jsonb,
    updated_at = now()
where id = 1025;
...
set name = '红毛榴莲水',
    ...
    tags = array['果香', '清爽'],
    translations = $${
      "en": {"name":"Soursop Drink","description":"Sweet tropical soursop flavor made into a refreshing fruit drink.","detail":"Made with soursop fruit flavor for a naturally tropical aroma and gentle sweetness. Best served chilled, refreshing with hot dishes or after a meal.","category":"Herbal Drinks","tags":["Fruity","Refreshing"]},
      "th": {"name":"น้ำทุเรียนเทศ","description":"เครื่องดื่มผลไม้รสทุเรียนเทศ หอมหวานสดชื่น","detail":"ปรุงด้วยรสผลทุเรียนเทศ ให้กลิ่นผลไม้เมืองร้อนและความหวานนุ่ม ดื่มเย็นแล้วสดชื่น เหมาะกับอาหารจานร้อนหรือหลังมื้ออาหาร","category":"เครื่องดื่มสมุนไพร","tags":["กลิ่นผลไม้","สดชื่น"]},
      "vi": {"name":"Nước mãng cầu xiêm","description":"Hương mãng cầu xiêm nhiệt đới, thanh mát và ngọt dịu.","detail":"Pha với hương vị mãng cầu xiêm, có mùi trái cây nhiệt đới tự nhiên và vị ngọt nhẹ. Uống lạnh rất thanh mát, hợp dùng với món nóng hoặc sau bữa ăn.","category":"Đồ uống thảo mộc","tags":["Vị trái cây","Thanh mát"]}
    }$$::jsonb,
    updated_at = now()
where id = 1039;
...
insert into public.menu_item_translations (
  item_id, lang, name, description, detail, category_label, tags, option_groups
)
select
  item.id,
  translation.key,
  coalesce(translation.value ->> 'name', item.name),
  coalesce(translation.value ->> 'description', item.description),
  coalesce(translation.value ->> 'detail', item.detail),
  translation.value ->> 'category',
  coalesce((select array_agg(tag) from jsonb_array_elements_text(coalesce(translation.value -> 'tags', '[]'::jsonb)) as tag), '{}'::text[]),
  coalesce(translation.value -> 'optionGroups', '[]'::jsonb)
from public.menu_items as item
cross join lateral jsonb_each(item.translations) as translation(key, value)
where translation.key in ('en', 'th', 'vi')
