          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3 md:hidden">
              <div className="grid gap-3">
                {filteredCampaigns.map(campaign => <article key={campaign.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-black text-slate-950">{campaign.title}</h3><p className="mt-1 font-mono text-xs text-[#C7A46A]">{campaign.code}</p></div><StatusBadge status={campaign.status} /></div>
                  <div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-slate-200"><div className="bg-slate-50 p-3"><p className="text-[10px] text-slate-400">优惠规则</p><p className="mt-1 text-xs font-bold text-slate-700">{campaignRule(campaign)}</p></div><div className="bg-slate-50 p-3"><p className="text-[10px] text-slate-400">有效期</p><p className="mt-1 text-xs font-bold text-slate-700">{campaign.validityDays ? `领取后 ${campaign.validityDays} 天` : formatRange(campaign.validFrom, campaign.validUntil)}</p></div></div>
                  <p className="mt-3 text-xs font-bold text-slate-500">已发放 {campaign.issued} · 已核销 {campaign.used}</p>
                  <div className="mt-3 flex gap-2">{canManageCampaigns && campaign.status !== 'ended' && (campaign.status === 'active' ? <button type="button" onClick={() => void changeCampaignStatus(campaign, 'paused')} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-slate-500" aria-label="暂停活动"><CirclePause size={16} /></button> : <button type="button" onClick={() => void changeCampaignStatus(campaign, 'active')} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 text-emerald-600" aria-label="上线活动"><CirclePlay size={16} /></button>)}<button type="button" onClick={() => { setIssueCampaignId(campaign.id); setTab('issue'); }} className="flex h-10 flex-1 items-center justify-center gap-1 rounded-xl bg-slate-950 text-xs font-bold text-white">发放优惠券<ChevronRight size={14} /></button></div>
                </article>)}
                {!filteredCampaigns.length && <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-16 text-center text-sm font-bold text-slate-400">暂无优惠券活动</div>}
              </div>
            </div>
...
function RecordsTable({ records }: { records: CouponRecord[] }) {
  return <div className="min-h-0 flex-1 overflow-auto"><div className="grid gap-3 p-3 md:hidden">{records.map(record => <article key={record.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-black text-slate-950">{record.customerName}</h3><p className="mt-1 text-xs text-slate-500">{record.phone}</p></div><RecordBadge status={record.status} /></div><div className="mt-3 rounded-xl bg-slate-50 p-3"><p className="font-bold text-slate-700">{record.couponTitle}</p><p className="mt-1 font-mono text-xs text-[#C7A46A]">{record.couponCode}</p></div><div className="mt-3 grid grid-cols-2 gap-3 text-xs"><div><p className="text-slate-400">订单号</p><p className="mt-1 break-all font-mono font-bold text-slate-600">{record.orderNo || '—'}</p></div><div><p className="text-slate-400">发放时间</p><p className="mt-1 font-bold text-slate-600">{formatDate(record.issuedAt)}</p></div></div><p className="mt-3 border-t border-slate-100 pt-3 text-xs text-slate-500">{record.usedAt ? `核销于 ${formatDate(record.usedAt)}` : record.expiresAt ? `有效至 ${formatDate(record.expiresAt)}` : '长期有效'}</p></article>)}{!records.length && <div className="rounded-2xl border border-dashed border-slate-200 px-4 py-16 text-center text-sm font-bold text-slate-400">暂无发放记录</div>}</div><table className="hidden w-full min-w-[900px] border-collapse text-sm md:table"><thead className="sticky top-0 z-10 bg-slate-50 text-xs text-slate-500"><tr><th className="px-5 py-3 text-left">用户</th><th className="px-4 py-3 text-left">优惠券</th><th className="px-4 py-3 text-center">状态</th><th className="px-4 py-3 text-left">订单号</th><th className="px-4 py-3 text-left">发放时间</th><th className="px-4 py-3 text-left">核销/过期</th></tr></thead><tbody className="divide-y divide-slate-100">{records.map(record => <tr key={record.id}><td className="px-5 py-4"><p className="font-bold text-slate-950">{record.customerName}</p><p className="mt-1 text-xs text-slate-500">{record.phone}</p></td><td className="px-4 py-4"><p className="font-bold text-slate-700">{record.couponTitle}</p><p className="mt-1 font-mono text-xs text-[#C7A46A]">{record.couponCode}</p></td><td className="px-4 py-4 text-center"><RecordBadge status={record.status} /></td><td className="px-4 py-4 font-mono text-xs text-slate-600">{record.orderNo || '—'}</td><td className="px-4 py-4 text-slate-500">{formatDate(record.issuedAt)}</td><td className="px-4 py-4 text-slate-500">{record.usedAt ? formatDate(record.usedAt) : record.expiresAt ? formatDate(record.expiresAt) : '长期有效'}</td></tr>)}{!records.length && <tr><td colSpan={6} className="px-4 py-16 text-center text-sm font-bold text-slate-400">暂无发放记录</td></tr>}</tbody></table></div>;
}
